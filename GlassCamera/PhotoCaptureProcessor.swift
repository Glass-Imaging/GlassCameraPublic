...
extension PhotoCaptureProcessor: AVCapturePhotoCaptureDelegate {
    ...
    func saveToPhotoLibrary() {
        ...
        if let capturedImage = self.capturedImage {
            Task {
                try! await photoCollection.addImage(capturedImage, alternateResource: self.dngFile, location: self.location)
                completeTransaction()
                self.dngFile = nil
            }
        }
        ...
        if let procesedImage = self.procesedImage {
            if let cgImageSource = CGImageSourceCreateWithData(procesedImage as CFData, nil),
               ...
               let heicData = cgImage.heic(withMetadata: self.imageMetadata) {
                ...
                Task {
                    try! await photoCollection.addImage(heicData, alternateResource: nil, location: self.location)
                    completeTransaction()
