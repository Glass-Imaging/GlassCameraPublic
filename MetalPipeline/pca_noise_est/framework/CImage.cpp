...
CImage::CImage(int Nx, int Ny, int bits, int num_channels) {
    this->unique_id++;
    this->id = this->unique_id;
    //
    this->Nx = Nx;
...
CImage::~CImage() {
    ...
    if (this->channels_id != NULL) delete[] this->channels_id;
}
...
void CImage::load(char* filename) {
    ...
    PRINT_VERBOSE("Loaded image \"%s\", %dx%d, %d channels, %d bits in I_%d\n", filename, this->Nx, this->Ny,
                  this->get_num_channels(), this->get_bits_per_channel(), this->id);
}
...
void CImage::save(char* filename) { this->save(filename, 8); }
...
void CImage::load_LUM(char* filename) {
    ...
    this->Nx = lumWidthArray[0] + 256 * lumWidthArray[1] + 65536 * lumWidthArray[2] + 16777216 * lumWidthArray[3];
    dummy += fread(&lumHeightArray, sizeof(char), 4, infile);
    dummy = dummy;  // To prevent the warning
    this->Ny = lumHeightArray[0] + 256 * lumHeightArray[1] + 65536 * lumHeightArray[2] + 16777216 * lumHeightArray[3];

    // Identify file format
    char lumFormat[5];
    dummy = fread(lumFormat, 1, 5, infile);
    if (strcmp(lumFormat, "12LI") != 0 && strcmp(lumFormat, "16LI") != 0 && strcmp(lumFormat, "16LU") != 0) {
...
static void* read_png_abort(FILE* fp, png_structp* png_ptr_p, png_infop* info_ptr_p) {
    ...
    if (NULL != fp && stdin != fp) (void)fclose(fp);
    return NULL;
}
...
void CImage::load_PNG(char* filename) {
    ...
    row_pointers = (png_byte**)png_malloc(png_ptr, this->Ny * sizeof(png_byte*));

    for (int y = 0; y < this->Ny; y++) {
        png_byte* row = (png_byte*)png_malloc(png_ptr, sizeof(uint8_t) * bytes_per_row);
        row_pointers[y] = row;
    }
...
void CImage::save_PNG(char* filename, int bits_per_channel) {
    ...
    color_type = (this->get_num_channels() == 1 || bit_depth < 8 ? PNG_COLOR_TYPE_GRAY : PNG_COLOR_TYPE_RGB);

    int height = this->get_height();
    int width = this->get_width();

    png_set_IHDR(png_ptr, info_ptr, this->get_width(), height, bit_depth, color_type, PNG_INTERLACE_NONE,
    ...
    if (bits_per_channel == 8) {
        for (int ch = 0; ch < channels; ch++) {
            ...
            for (int y = 0; y < height; y++) {
                for (int x = 0; x < this->get_width(); x++) row_pointers[y][x * channels + ch] = orig[width * y + x];
            }
        }
    }
...
int CImage::get_num_channels() { return this->num_channels; }

int CImage::get_width() { return this->Nx; }

int CImage::get_height() { return this->Ny; }

int CImage::get_bits_per_channel() { return this->bits; }
