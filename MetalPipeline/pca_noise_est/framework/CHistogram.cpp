...
class CHistogram {
    ...
    */
    CHistogram<T>(int bins, T* data, float* datal, int N);
...
CHistogram<T>::CHistogram(int bins, T* data, float* datal, int N) {
    ...
    for (int idx = 0; idx < N; idx++) {
        ...
        if (idx == N - 1 || elements_count == samples_per_bin) {
            ...
            memcpy(this->data_bins[bin], buffer, sizeof(T) * elements_count);

            memcpy(this->datal_bins[bin], bufferl, sizeof(float) * elements_count);
