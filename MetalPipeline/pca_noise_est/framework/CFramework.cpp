...
CFramework::~CFramework() {
    ...
    //
    if (CFramework_verbose) this->print_matrix_list();
    //
    ...
    if (CFramework::CFramework_ref != NULL) {
        if (CFramework_verbose) PRINT_VERBOSE("Deleting singleton framework object\n");
...
CFramework* CFramework::get_framework() {
    if (CFramework::CFramework_ref == NULL) {
        ...
        if (CFramework_verbose) PRINT_VERBOSE("Creating singleton framework object\n");
    }
...
void CFramework::set_verbose(bool verbose) { CFramework::CFramework_verbose = verbose; }

float* CFramework::create_matrix(int num_dims, int* N, int& out_id) {
    int product_dims = 1;
    for (int i = 0; i < num_dims; i++) product_dims *= N[i];
...
int CFramework::find_matrix(float* data) {
    ...
    for (map<int, MatrixInfo*>::iterator it = this->mmatrices.begin(); it != end; ++it) {
        MatrixInfo* matrix_info = it->second;
        if (matrix_info->data == data) return matrix_info->id;
    }
    return -1;
}
...
void CFramework::save_matrix(MatrixInfo* matrix, char* filename) {
    ...
    int num_elements = 1;
    for (int i = 0; i < matrix->num_dims; i++) num_elements *= matrix->N[i];

    fwrite(&matrix->num_dims, sizeof(matrix->num_dims), 1, fp);
    for (int i = 0; i < matrix->num_dims; i++) fwrite(&matrix->N[i], sizeof(matrix->N[0]), 1, fp);
...
int CFramework::load_matrix(MatrixInfo* matrix, char* filename) {
    ...
    int num_elements = 1;
    for (int i = 0; i < num_dims; i++) num_elements *= matrix->N[i];
...
void CFramework::print_backtrace(FILE* out) {
    ...
    for (size_t i = 1; i < stack_depth; i++) fprintf(out, "    %s\n", stack_strings[i]);
