...
void printusage(const char* pname, const char* gp, vector<OptStruct*>& opt, vector<ParStruct*>& par) {
    // int nopt = opt.size();
    int npar = par.size();
    ...
    for (int i = 0; i < npar; i++) printf(" %s ", par[i]->name);
...
int parsecmdline(const char* pname, const char* function, const int argc, char** argv, vector<OptStruct*>& opt,
    ...
    int i = 0;
    for (int index = optind; index < argc; index++, i++) par[i]->value = argv[index];
