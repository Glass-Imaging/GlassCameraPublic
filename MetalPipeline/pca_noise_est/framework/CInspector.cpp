...
int CInspector::read_tokens(vector<string>* tokens, string input) {
    ...
    do {
        string sub;
        iss >> sub;
        if (sub.c_str() != NULL && sub != "") tokens->push_back(sub);
    } while (iss);
...
void list_of_ints(char* dest, const char* separator, int* numbers, int N) {
    ...
    for (int i = 0; i < N; i++) {
        ...
        strcat(dest, buffer);
        if (i != N - 1) strcat(dest, separator);
    }
}
...
int CInspector::process_input(string input, CFramework* fw) {
    ...
    if (command == "save") {
        if (G_using_matrix == NULL) return INSPECTOR_ERR_NO_MATRIX_SELECTED_YET;
        if (tokens.size() != 2) return INSPECTOR_ERR_NO_FILENAME;
        //
    ...
    if (command == "load") {
        ...
        switch (status) {
            case 0:
                ...
                break;
            case -1:
                return INSPECTOR_ERR_CAN_NOT_OPEN_FILE;
                break;
            case -2:
                return INSPECTOR_ERR_INCOMPATIBLE_DIMENSIONS;
                break;
            case -3:
                return INSPECTOR_ERR_READ_ERROR;
                break;
        }
        return INSPECTOR_OK;
    }
...
void CInspector::inspector(CFramework* fw, const char* file, int line) {
    ...
    while (input != "q" && input != "quit" && input != "exit") {
        ...
        if (err_code != INSPECTOR_OK) process_error(err_code);
    }
